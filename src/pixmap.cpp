...
pixmap::pixmap(const std::shared_ptr<renderer> renderer,
  ...
  if (result != AVIF_RESULT_OK) {
    throw std::runtime_error(fmt::format("[avifDecoderSetIOMemory] error while "
  ...
  if (result != AVIF_RESULT_OK) {
    throw std::runtime_error(fmt::format(
        "[avifDecoderParse] error while parsing AVIF: {}, error: {}", filename,
  ...
  if (avifImageYUVToRGB(decoder->image, &rgb) != AVIF_RESULT_OK) {
    throw std::runtime_error(
        fmt::format("[avifImageYUVToRGB] error while converting YUV to RGB on "
                    "avifImage: {}",
                    filename));
  }
...
void pixmap::draw(const geometry::point &point, const double_t angle,
  ...
  SDL_RenderCopyEx(*_renderer, _texture.get(), nullptr, &rect, angle, nullptr,
                   static_cast<SDL_RendererFlip>(flip::none));
}
